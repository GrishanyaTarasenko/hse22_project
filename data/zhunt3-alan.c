...
void show_probability(unsigned seqlength, float *dl, float *slope, float *probability, char *sequence, char **antisyn,
                      char *filename, int low, int high);
...
unsigned input_sequence(FILE *file, int nucleotides, int showfile) {
    ...
    while (j = 0, fgets(tempstr, 128, file) != NULL) {
        while ((c = tempstr[j++]) != 0) {
            if (c == 'a' || c == 't' || c == 'g' || c == 'c' || c == 'n' || c == 'A' || c == 'T' || c == 'G' ||
                c == 'C' || c == 'N') {
                length++;
            }
        }
